...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    // Log in with the standard demo user
    const ok = login('alex@example.com', 'password123');
    setLoading(false);
    if (ok) {
      navigate('/dashboard');
    } else {
      ...
      <div className="w-full lg:w-5/12 flex flex-col justify-center items-center px-6 sm:px-12 xl:px-16 py-12 bg-white shadow-2xl z-10">
        <div className="w-full max-w-md">
          ...
          <button
            ...
            className="w-full mb-6 py-3 px-4 bg-white border border-[#c3c6d7] hover:border-[#0057d9] hover:bg-[#0057d9]/5 text-[#191c1e] rounded-xl text-sm font-semibold flex justify-center items-center gap-2.5 shadow-sm active:scale-[0.99] transition-all duration-200 cursor-pointer"
          >
